...
Tr <- exp(as.matrix(mus.collapsed))
Ir <- as.numeric(df_is)
Im <- 6.228*10**9
...
pc.plot1 <- ggplot(data.frame(sdev=datCAZ.pca$sdev/sum(datCAZ.pca$sdev)), aes(y=sdev, x=(1:length(sdev)))) + geom_point() + labs(x="Principal Component", y="Variance Explained (%)", title="Variance Explained") + theme_classic()
...
dd2 <- cbind(t(d_corr_norm), cell, prot, monosacch)
